...
class UNet(nn.Module):
    def __init__(self, img_channels=9, base_channels=64, out_channel = 1, channel_mults=(1, 2, 4), num_res_blocks=2, time_dim=64 * 4, activatedfun=nn.SiLU, dropout=0.1):
        super().__init__()
        ...
        self.last_layer = nn.Sequential(
            activatedfun(),
            nn.Conv2d(base_channels, out_channel, 3, padding=1)
        )

    def forward(self, x, time):
        time_emb = self.time_embedding(time) if time is not None else None
        x = self.init_conv(x)
...
if __name__ == '__main__':
    ...
    t = torch.randint(0, 2000, (1,)).cuda()
    a = net(input, None)
    print(net(input, None).shape)